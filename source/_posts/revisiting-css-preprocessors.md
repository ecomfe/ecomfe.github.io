...
* 没有变量和合理的样式复用机制，使得逻辑上相关的属性值必须以**直接量**的形式重复输出，导致难以维护。
...
Less 的基本语法属于「CSS 风格」，而 Sass、Stylus 相比之下激进一些，利用缩进、空格和换行来减少需要输入的字符。不过区别在于 Sass、Stylus 同时也兼容「CSS 风格」代码。多一种选择在更灵活的同时，在团队开发中也免不了增加更多约定来保持风格统一。而对个人而言，语法风格按自己口味选择即可。（**后面的 Sass 代码会用被更多人接受的 SCSS 风格给出。**）
...
2. 定义一个不输出的样式片段（可以输入参数），在目标位置输出。（后面如无特殊说明，mixin 均用来指代此类混入。）
...
Less 中的 mixin 通过「guard」的方式支持简单的条件分支控制。比如我们要实现一个控制 `::placeholder` 样式的 mixin，当传入颜色时只设置颜色，当传入 ruleset 变量时输出对应的样式规则，其他情况输出一个默认的 `color`：
