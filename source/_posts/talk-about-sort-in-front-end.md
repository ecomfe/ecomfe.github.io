...
- 更高的计算效率。`快速排序` 由于算法时间复杂度常数因素的优势，比 `归并排序` 要快约*23%*（不命中最差组合的情况下）
...
浏览器原生接口的实现可能存在差异，但是也有规律可循。从目前已知的情况来看，所有主流浏览器（包括IE6，7，8）对于数组排序算法的实现主要分为两类：

1. 稳定排序算法。包括 `归并排序`、`Timsort` 等
2. 不稳定排序算法。基本就只是 `快速排序`

所以，我们只要将快速排序经过定制改造，变成稳定排序的是不是就可以了？

一般来说，对对象数组采用不稳定排序会对结果有影响。字符串数组本身使用稳定排序或不稳定排序，最后结果均相等。因此方案大致如下：

    将待排序数组进行预处理，为每个待排序的对象增加自然序属性，不与对象的其他属性冲突即可。
    自定义排序比较方法compareFn，总是将自然序作为前置判断相等时的第二判断维度。

以上方案涉及对传入对象的改造。由于不仅修改了用户数据，而且需要增加额外空间用于存储自然序属性。可想而知，作为引擎的 `v8` 不会采用类似的手段。但是用户自行定制排序算法是可行的方案。
