...
尽管如此，这个项目中的 Native Bridge 对我来说还是很神秘的，它到底是通过什么『魔法』打通了两种不同编程语言之间的边界。正因为缺少对这部分内容的了解，才促使我花了一些时间做了这些调研，然后把我的理解通过这几篇文章分享给大家。
...
毋庸置疑，在没有一个对项目清晰的规划设计之前就写代码是没有任何意义的，架构设计对于软件开发来说是很重要的一步，因此我会尽量清晰的介绍我的想法以及为什么要做某个决定，以及背后的原因。
...
我默认主要的执行逻辑是用 C++ 写的，它内部有一个自定义全局上下文的JS引擎（基本上所有的 JS引擎 都提供扩展内置函数的能力）。所以当看到自定义全局上下文的时候不要惊讶，其实并没有什么特殊的地方，只是通过 C++ 扩充了一些内置的 JS 函数。稍候我们会介绍这部分的内容。
...
一旦我们调用这些自定义的JS函数，JS引擎就会执行我们在 C++ 里面实现的那部分代码，然后就可以继续调用 Objective C 的代码来呈现UI。听起来有点儿复杂，但是不要灰心，很快就看到曙光了。
...
上述所有的逻辑都可以在同一个线程（主线程）中运行，但是可能会导致一些性能问题。为了避免可能出现的性能问题，我们决定用一个新的线程来处理 Objective C 和 UI 的问题（*没看太懂*）

![2.png](how-to-create-you-own-native-bridge/2.png)

Even though this approach makes sense, it doesn’t work.

尽管听起来比较合理，实际情况却不是这样子的。
...
问题在于 Apple 限制了只可以在主线程中渲染 UI，这也就导致我们只能在主线程中去执行 Objective C 的代码。一旦这么设计的话，就导致应用的入口点和平台绑定在了一起(*没太看懂*)，至少会导致切换到其它平台的时候有非常复杂的工作。所以我们需要从头考虑我们整体的架构设计，看看是否可以进行一些改进来满足我们的需求。
...
如果必须在主线程中去执行 Objective C 的代码来渲染UI，那我们就服从 Apple 的这个限制，不过我们可以在程序启动的时候，启动一个新的线程来初始化 JS引擎，然后运行我们打包好的 JS 代码。正如前面提到的，如果我们使用 C++ 代码扩充了一些内置的 JS 函数，一旦这些函数被调用，我们就可以发送一些命令到主线程，然后在主线程通过一些逻辑来接收这些命令，进而来绘制 UI。
...
在主线程，Objective C 的代码会接收 JS线程 发送过来的命令，然后执行对应的 UI 绘制逻辑。如果成功处理了发送过来的命令，Objective C 会回调一个从 C++ 传递过来的函数（如果有的话，类似 JS 里面的 callback 机制）
...
1. 因为需要在主线程中绘制UI，所以我们需要一个 Cocoa 类型的应用
...
2. 当程序启动的时候，我们需要创建一个 JS线程 来初始化 JS引擎，然后执行我们打包在一起的 JS代码。
...
之后，编辑 `AppDelegate.m` 文件里面的 `applicationDidFinishLaunching` 函数，在这个函数里面应该启动一个新的线程来初始化 JS引擎，代码如下：
...
关于 Selector 的问题，建议还是参考[Apple的官方文档](https://developer.apple.com/library/content/documentation/General/Conceptual/DevPedia-CocoaCore/Selector.html)，简单来说就是选择一个对象可以执行的方法？(*没太看懂*)
...
正如在代码中看到的，我这里用了 ChakraCore 而没有选择 V8 的原因是：
...
ChakraProxy 的逻辑很简单，简单来说就是用来整合 Objective C 和 C++ 代码的一个容器：
...
显而易见，ChakraProxy 除了输出一行日志之外，什么也没有做，但是我们可以在此基础上开展更多的工作了。在下一篇我们会添加更多的功能，比如整合 ChakraCore，基本的桥接模型，以及其它的一些内容。
